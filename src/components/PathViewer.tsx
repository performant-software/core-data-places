  ...
  return (
    ...
    >
      <div className="w-full h-screen flex flex-row relative">
        {path && (
          <div className="absolute bottom-[15%] left-[50%] -translate-x-1/2 mx-auto w-48 h-16 rounded-full bg-white z-[999] drop-shadow-xl flex justify-around items-center hover:scale-110 transition">
            <ArrowUturnLeftIcon
        ...
        <div className="h-full w-1/2">
          {path && current >= 0 ? (
            <CoreDataPlace
              placeURIs={[
                `${import.meta.env.PUBLIC_CORE_DATA_API_URL}/${path.path[current].place.uuid}?project_ids=${import.meta.env.PUBLIC_CORE_DATA_PROJECT_ID}`,
              ]}
            ...
            path && (
              <CoreDataPlace
                placeURIs={path.path.map(
                  (current) =>
                    `${import.meta.env.PUBLIC_CORE_DATA_API_URL}/${current.place.uuid}?project_ids=${import.meta.env.PUBLIC_CORE_DATA_PROJECT_ID}`
                )}
                animate={false}
                mapId="cover"
              />
